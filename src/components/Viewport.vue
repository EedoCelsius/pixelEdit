  ...
  <div ref="viewportEl" class="relative flex-1 min-h-0 p-2 overflow-hidden touch-none"
    ...
    <div id="stage" class="absolute shadow-inner select-none touch-none"
      ...
      <svg v-show="viewportStore.display==='result'" class="absolute w-full h-full top-0 left-0 pointer-events-none block" :viewBox="viewportStore.viewBox" preserveAspectRatio="xMidYMid meet">
        <RenderNode v-for="node in renderTree" :key="node.id" :node="node" />
      </svg>
      <!-- 그리드 -->
      <svg class="absolute w-full h-full top-0 left-0 pointer-events-none block" :viewBox="viewportStore.viewBox" preserveAspectRatio="xMidYMid meet">
...
const { viewport: viewportStore, layers, layerGroups, viewportEvent: viewportEvents } = useStore();
...
const renderTree = computed(() => {
    const build = (node) => {
        if (typeof node === 'number') {
            const props = layers.getProperties(node);
            return { type: 'layer', id: node, props };
        } else {
            return { type: 'group', id: node.id, visibility: node.visibility, children: node.children.map(build) };
        }
    };
    return layerGroups.tree.map(build);
});
    ...
    setup(props) {
        return () => {
            if (props.node.type === 'group') {
                return h('g', { visibility: props.node.visibility ? 'visible' : 'hidden' }, props.node.children.map(child => h(RenderNode, { node: child })));
            } else {
                const p = props.node.props;
                return h('path', {
                    d: layers.pathOf(p.id),
                    fill: rgbaCssU32(p.color),
                    'fill-rule': 'evenodd',
                    'shape-rendering': 'crispEdges',
                    visibility: p.visibility ? 'visible' : 'hidden'
                });
            }
        };
    }
