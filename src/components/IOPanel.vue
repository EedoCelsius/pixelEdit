<template>
  <div class="flex items-center justify-between p-2">
    <div class="flex items-center gap-2">
      <input ref="fileInput" type="file" accept="image/*,application/json" class="hidden" @change="onFileChange" />
...
async function onFileChange(e) {
  ...
  if (file.type === 'application/json' || file.name.endsWith('.json')) {
    try {
      ...
      if (data?.input && data?.state) {
        await input.load(data.input.src);
        output._apply(JSON.stringify(data.state));
      }
    } catch (err) {
      console.error(err);
    }
