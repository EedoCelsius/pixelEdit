<template>
  <div v-if="stageResizeService.show" class="fixed inset-0 flex items-center justify-center bg-black/50">
    <div class="bg-slate-800 px-16 py-8 rounded-md text-xs space-y-2">
      <div class="relative w-56 h-56 mx-auto">
        <label class="absolute -top-8 left-1/2 -translate-x-1/2 flex flex-col items-center">
          <span class="mb-1">Top</span>
          <input type="number" v-model.number="top" class="w-16 px-1 py-0.5 rounded bg-slate-700" />
        </label>
        <label class="absolute top-1/2 -left-16 -translate-y-1/2 flex flex-col items-center">
          <span class="mb-1">Left</span>
          <input type="number" v-model.number="left" class="w-16 px-1 py-0.5 rounded bg-slate-700" />
        </label>
        <label class="absolute top-1/2 -right-16 -translate-y-1/2 flex flex-col items-center">
          <span class="mb-1">Right</span>
          <input type="number" v-model.number="right" class="w-16 px-1 py-0.5 rounded bg-slate-700" />
        </label>
        <label class="absolute -bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center">
        ...
        <div class="absolute inset-0 border-2 border-dashed border-slate-500">
          <button @click="adjust('top', 1)" class="absolute -top-4 left-1/2 -translate-x-1/2 w-4 h-4 bg-slate-700 rounded text-white">▲</button>
          <button @click="adjust('top', -1)" class="absolute top-1 left-1/2 -translate-x-1/2 w-4 h-4 bg-slate-700 rounded text-white">▼</button>
          <button @click="adjust('bottom', 1)" class="absolute -bottom-4 left-1/2 -translate-x-1/2 w-4 h-4 bg-slate-700 rounded text-white">▼</button>
          ...
          <button @click="adjust('left', 1)" class="absolute top-1/2 -left-4 -translate-y-1/2 w-4 h-4 bg-slate-700 rounded text-white">◀</button>
          <button @click="adjust('left', -1)" class="absolute top-1/2 left-1 -translate-y-1/2 w-4 h-4 bg-slate-700 rounded text-white">▶</button>
          <button @click="adjust('right', 1)" class="absolute top-1/2 -right-4 -translate-y-1/2 w-4 h-4 bg-slate-700 rounded text-white">▶</button>
