...
function onKeydown(event) {
  ...
  switch (event.key) {
    ...
    case 'ArrowUp':
      ...
      if (shift && !ctrl) {
        if (!layers.hasSelection) return;
        const newTail = layers.aboveId(layerPanel.tailId) ?? layers.uppermostId;
        rangeSelect.selectRange(layerPanel.anchorId, newTail);
    ...
    case 'Backspace':
      ...
      rangeSelect.selectOne(newSelect);
  ...
  if (ctrl) {
    if (key === 'a') {
      ...
      rangeSelect.replaceSelection(layers.order, anchor, tail);
    } else if (key === 'z' && !shift) {
      event.preventDefault();
      output.undo();
